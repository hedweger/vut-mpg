...
float ridiciBody[POCET_RIDICICH_BODU][3] = {
    {60, 400, 0.0}, {300, 300, 0.0}, {150, 200, 0.0}, {60, 60, 0.0}};
...
void bitmapText() {
  ...
  for (int i = 0; i < POCET_RIDICICH_BODU; i++) {
    str = "Ridici bod: " + std::to_string(i) +
          " X: " + std::to_string(int(ridiciBody[i][0])) +
          " Y: " + std::to_string(int(ridiciBody[i][1]));
...
void onMouse(int button, int state, int mx, int my) {
  ...
  if (button == GLUT_LEFT_BUTTON && bezier == true) {
    if (state == GLUT_UP) {
      ...
      for (int i = 0; i < POCET_RIDICICH_BODU; i++) {
        if ((mx > ridiciBody[i][0] - 10 && mx < ridiciBody[i][0] + 10) &&
            (my > ridiciBody[i][1] - 10 && my < ridiciBody[i][1] + 10)) {
          aktBod = i;
        }
      }
    }
...
void createMenu(void (*func)(int value)) {
  ...
  int bezrectSub = glutCreateMenu(func);
  ...
  int timerSub = glutCreateMenu(func);
  ...
  glutAddSubMenu("Bezier/Rect", bezrectSub);
  // Ukol 4 aktivujte submenu pro zmenu zapnuti/vypnuti timeru
  glutAddSubMenu("Timer", timerSub);
...
int main(int argc, char *argv[]) {
  ...
  glutInitWindowSize(
      winWidth,
      winHeight); // nastaveni pocatecni velikosti dale oteviranych oken
